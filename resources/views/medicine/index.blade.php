		...
		<table id="datatables-2" class="table table-striped table-bordered table-hover">
			<thead>
				<tr>
					...
					<th>{!! __('module.table.medicine.code') !!}</th>
					<th width="100px">{!! __('module.table.medicine.price') !!}</th>
					<th>{!! __('module.table.medicine.usage') !!}</th>
			...
			<tbody>
				@foreach($medicines as $i => $medicine)
					@php
						$record_locked = $medicine->recordLocked();
					@endphp
					<tr>
						<td class="text-center">{{ ++$i }}</td>
						<td>{!! $record_locked ? '<i class="fa fa-lock fa-fw"></i> ' : '' !!}{{ $medicine->name }}</td>
						<td>{{ $medicine->code }}</td>
						...
						<td class="text-center">
							...
							@can('Medicine Edit')
							<a href="{{ route('medicine.edit', $medicine->id) }}" class="btn btn-info btn-xs btn-flat" data-toggle="tooltip" data-placement="left" title="{{ __('label.buttons.edit') }}"><i class="fa fa-pencil-alt"></i></a>
							@endcan

							@can('Medicine Delete')
								@if(!$record_locked)
									<button class="btn btn-danger btn-xs btn-flat BtnDeleteConfirm" value="{{ $medicine->id }}" data-toggle="tooltip" data-placement="left" title="{{ __('label.buttons.delete') }}"><i class="fa fa-trash-alt"></i></button>
